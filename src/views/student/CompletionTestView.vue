        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center h-16">
            ...
            <div v-if="currentQuestion" class="text-sm text-gray-500">
              Question {{ currentQuestion.progress }} of {{ currentQuestion.total_questions }}
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-if="!isLoading && currentQuestion && !testResults" class="bg-white shadow rounded-lg p-6">
          ...
          <div class="w-full bg-gray-200 rounded-full h-2.5 mb-6">
            <div 
              ...
              :style="`width: ${(currentQuestion.progress / currentQuestion.total_questions) * 100}%`"
            ></div>
          </div>
...
const hasRemainingQuestions = ref(true);
  ...
  try {
    ...
    // We already have the completionId from the route
    // No need to pass it as undefined
...
const submitAnswer = async () => {
  ...
  try {
    ...
    // FIX: Always check for more unanswered questions before deciding to finalize
    const unansweredResponse = await api.get(
      `/completion-test/${completionId.value}/unanswered-questions`
    );
    
    console.log('Unanswered questions:', unansweredResponse.data.unanswered_count);
...
const finalizeTest = async () => {
  try {
    ...
    try {
      ...
      console.log('Could not check for unanswered questions');
    }
...
const checkRemainingQuestions = async () => {
  try {
    ...
    console.log(`Remaining questions: ${response.data.unanswered_count - 1}`);
    
  } catch (err) {
    console.error('Failed to check remaining questions:', err);
